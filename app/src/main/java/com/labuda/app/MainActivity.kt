...
class MainActivity:ComponentActivity(){
 ...
 private val qrImport=registerForActivityResult(ActivityResultContracts.StartActivityForResult()){if(it.resultCode==RESULT_OK){val fromIntent=it.data?.dataString;val clipboard=getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager;val fromClip=clipboard.primaryClip?.getItemAt(0)?.coerceToText(this)?.toString();val value=fromIntent?.takeIf{s->s.isNotBlank()}?:fromClip?.takeIf{s->s.isNotBlank()};if(!value.isNullOrBlank())acceptQr(value)}}
 private val qrImagePick=registerForActivityResult(ActivityResultContracts.GetContent()){uri->if(uri!=null)decodeQrFromUri(uri)}
...
@Composable private fun ImportScreen(url:String,onUrl:(String)->Unit,busy:Boolean,message:String,onScanQr:()->Unit,onPickQrImage:()->Unit,onClipboard:()->Unit,onBack:(()->Unit)?,onImport:()->Unit){var showQrChoice by remember{mutableStateOf(false)};Box(Modifier.fillMaxSize()){if(onBack!=null){IconButton(onClick=onBack,modifier=Modifier.align(Alignment.TopStart).padding(8.dp)){Icon(Icons.Filled.ArrowBack,"Назад")}};Column(Modifier.fillMaxSize().padding(24.dp),horizontalAlignment=Alignment.CenterHorizontally,verticalArrangement=Arrangement.Center){Text("LBD",fontSize=56.sp,fontWeight=FontWeight.Black);Text("LABUDA",fontSize=18.sp,fontWeight=FontWeight.Bold);Spacer(Modifier.height(32.dp));Card(shape=RoundedCornerShape(24.dp)){Column(Modifier.padding(20.dp)){Text("Добавить подписку",fontSize=20.sp,fontWeight=FontWeight.Bold);Spacer(Modifier.height(12.dp));OutlinedTextField(url,onUrl,Modifier.fillMaxWidth(),label={Text("URL подписки или VLESS")},singleLine=true);Spacer(Modifier.height(12.dp));Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp)){Button({showQrChoice=true},Modifier.weight(1f)){Text("QR")};Button(onClipboard,Modifier.weight(1f)){Icon(Icons.Filled.ContentPaste,null);Spacer(Modifier.size(6.dp));Text("Буфер")}};Spacer(Modifier.height(12.dp));Button(onImport,Modifier.fillMaxWidth(),enabled=!busy&&url.isNotBlank()){Text(if(busy)"Загрузка…"else"Импортировать подписку")};if(message.isNotBlank())Text(message,Modifier.padding(top=10.dp),color=MaterialTheme.colorScheme.onSurfaceVariant)}}}
if(showQrChoice){AlertDialog(onDismissRequest={showQrChoice=false},title={Text("QR-код")},text={Column(verticalArrangement=Arrangement.spacedBy(8.dp)){Button({showQrChoice=false;onScanQr()},Modifier.fillMaxWidth()){Text("Сканировать QR-код")};Button({showQrChoice=false;onPickQrImage()},Modifier.fillMaxWidth()){Text("Вставить картинку QR-кода")}}},confirmButton={},dismissButton={TextButton({showQrChoice=false}){Text("Отмена")}})}}
}
}
@Composable private fun MainScreen(subs:List<SubscriptionInfo>,selected:VlessProfile?,connected:Boolean,busy:Boolean,message:String,stats:VpnStatsSnapshot,dark:Boolean,onDark:(Boolean)->Unit,onSettings:()->Unit,onSelect:(VlessProfile)->Unit,onConnect:()->Unit,onRefresh:()->Unit,onImport:()->Unit,onFavorite:(VlessProfile)->Unit){Column(Modifier.fillMaxSize().padding(horizontal=12.dp).padding(top=32.dp)){Row(Modifier.fillMaxWidth().padding(top=8.dp),verticalAlignment=Alignment.CenterVertically){Text("LABUDA",fontSize=24.sp,fontWeight=FontWeight.Black,modifier=Modifier.weight(1f));Icon(Icons.Filled.DarkMode,"Тёмная тема",Modifier.size(18.dp));Switch(dark,onDark);IconButton(onSettings){Icon(Icons.Filled.Settings,"Настройки")};IconButton(onRefresh,enabled=!busy){Icon(Icons.Filled.Refresh,"Обновить")};IconButton(onImport){Icon(Icons.Filled.Add,"Добавить")}};Spacer(Modifier.height(5.dp));Card(Modifier.fillMaxWidth(),shape=RoundedCornerShape(20.dp)){Column(Modifier.padding(10.dp),horizontalAlignment=Alignment.CenterHorizontally){Box(Modifier.size(82.dp).background(MaterialTheme.colorScheme.onSurfaceVariant,CircleShape),contentAlignment=Alignment.Center){Text("LBD",fontSize=28.sp,fontWeight=FontWeight.Black,color=MaterialTheme.colorScheme.surface)};Spacer(Modifier.height(5.dp));Text(if(connected)"Лабуда подключена"else"Лабуда отключена",fontSize=17.sp,fontWeight=FontWeight.Bold);Spacer(Modifier.height(6.dp));Button(onConnect,Modifier.fillMaxWidth().height(42.dp),shape=RoundedCornerShape(14.dp),enabled=selected!=null||subs.any{it.profiles.isNotEmpty()}){Text(if(connected)"Отключить"else"Подключить",fontSize=15.sp)}}};Spacer(Modifier.height(5.dp));VpnStatsCard(stats);Spacer(Modifier.height(6.dp));Row(Modifier.fillMaxWidth(),verticalAlignment=Alignment.CenterVertically){Text("Подписки (${subs.size})",fontSize=17.sp,fontWeight=FontWeight.Bold,modifier=Modifier.weight(1f));if(busy)Text("Обновление…",color=MaterialTheme.colorScheme.onSurfaceVariant)};Spacer(Modifier.height(4.dp));LazyColumn(verticalArrangement=Arrangement.spacedBy(7.dp)){subs.forEach{s->item(key="sub-${s.id}"){SubscriptionHeader(s)};items(s.profiles,key={"${s.id}:${it.raw}"}){p->ServerCard(p,selected,onSelect,onFavorite)}}};if(message.isNotBlank())Text(message,Modifier.padding(vertical=6.dp),color=MaterialTheme.colorScheme.onSurfaceVariant)}}
